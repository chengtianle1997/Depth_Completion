...
#parser.add_argument('--mod', type=str, default='erfnet', choices=Models.allowed_models(), help='Model for use')
parser.add_argument('--mod', type=str, default='hourglass', choices=Models.allowed_models(), help='Model for use')
...
parser.add_argument('--save_freq', type=int, default=10000, help="save every x interations")
...
def main():
    ...
    #Init Recoder
    csvrec = csvr.CsvRecoder(save_id)
    ...
    for epoch in range(args.start_epoch, args.nepochs):
        ...
        for i, (input, gt) in tqdm(enumerate(train_loader)):
            if args.mod == 'mod':
                ...
                #prediction, lidar_out, precise, guide = model(input, epoch)
                prediction, precise, guide = model(input, epoch)
                ...
                loss_guide = criterion_guide(guide, gt)
                #loss = args.wpred*loss + args.wlid*loss_lidar + args.wrgb*loss_rgb + args.wguide*loss_guide
                loss = args.wpred*loss + args.wrgb*loss_rgb + args.wguide*loss_guide
                ...
                prediction, lidar_out = model(input, epoch)

                loss = criterion_local(prediction, gt)
                loss_lidar = criterion_lidar(lidar_out, gt)
                #loss_rgb = criterion_rgb(precise, gt)
                #loss_guide = criterion_guide(guide, gt)
                loss = args.wpred*loss + args.wlid*loss_lidar
        ...
        print("===> Last best score was RMSE of {:.4f} in epoch {}".format(lowest_loss,
                                                                           best_epoch))
        #insert csv record
        csvrec.insertrow(epoch+1, losses.val, score_train.avg, score_train_1.avg, score_valid, score_valid_1, score_selection, score_selection_1)
    ...
    with torch.no_grad():
        ...
        if args.mod == 'mod':
            for i, (input, gt) in tqdm(enumerate(loader)):
                ...
                #save_rgb_input(i, input, epoch)
                #conditional_save_pred(i,prediction,epoch)
...
def conditional_save_pred(i, pred, epoch):
    output_directory = "Output"
    # save images for visualization/ testing
    image_folder = os.path.join(output_directory,"hourglass_erfnet")
    if not os.path.exists(image_folder):
        os.makedirs(image_folder)
    img = torch.squeeze(pred.data.cpu()).numpy()
    filename = os.path.join(image_folder, '{0:010d}.png'.format(i))
    img = (img[0]*256).astype('uint16')
    cv2.imwrite(filename, img)

def save_rgb_input(i, input, epoch):
    output_directory = "Output"
    # save images for visualization/ testing
    image_folder = os.path.join(output_directory,"rgb_image")
    if not os.path.exists(image_folder):
        os.makedirs(image_folder)
    input = torch.squeeze(input.data.cpu()).numpy()
    filename = os.path.join(image_folder, '{0:010d}.png'.format(i))
    width, height = input.shape[3], input.shape[2]
